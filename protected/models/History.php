...
class History extends MRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array(
                'itemid, manageid, sid, sname, add_time, last_time, note, count, status, typeid',
                'safe',
                'on' => 'search'
            ) ,
        );
    ...
    public static function addCount($sid) {
        ...
        if ($member != null) {
            ...
            $m = self::model()->findByAttributes(array(
                'sid' => $sid
            ));
            $time = time();
            if ($m != null) {
    ...
    public static function upMCount() {
        ...
        $list = History::model()->findAllByAttributes(array(
            'manageid' => $manageid
        ));
        $time = time();
        if (count($list) > 0) {
