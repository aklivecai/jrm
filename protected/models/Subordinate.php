...
class Subordinate extends CActiveRecord
{
	...
	public function getNotUser($q=false)
	{
		...
		$sql = strtr('SELECT manageid AS itemid,user_nicename,user_name AS title,fromid FROM :tabl  WHERE  :sql',
	...
	{
		$mod = self::iniMods();
		$sql = strtr('manageid IN (SELECT mid FROM :tabl WHERE fromid=:fromid AND  manageid=:manageid) OR  manageid IN (SELECT manageid FROM :tabl_manage WHERE fromid=:fromid AND branch=:branch AND isbranch=0)',
			array(
				':tabl'=>self::$table
	...
	{
		return array(
			array('manageid', 'required'),
			array('manageid,mid,fromid,branch,isbranch', 'numerical', 'integerOnly'=>true),			
		);
	}    
