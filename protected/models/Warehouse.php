...
class Warehouse extends LRecord
{
	...
	{
		return array(
			array('itemid','addNow','on'=>'create'),
			array('fromid','autoID'),
			array('itemid, fromid, name','required'),
			array('itemid, fromid', 'length', 'max'=>25),
			array('name,user_name', 'length', 'max'=>64),
			array('telephone', 'length','max'=>50),
			array('note', 'length', 'max'=>255),
			array('listorder','length','max'=>10),
			
			array('itemid, fromid, listorder', 'numerical', 'integerOnly'=>true),

			array('name','checkRepetition'),
		);
	}
