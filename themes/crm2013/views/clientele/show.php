...
$items = array();
$items[] = array('icon'=>'isw-up_circle','label'=>'捞起来','url'=>array('getSeas','id'=>$model->itemid));
foreach ($model->getNP(true) as $key => $value) {
  $items[] = array('icon'=>'isw-bookmark','label'=>Tk::g($key),'url'=>array('showSeas','id'=>$value));
}
  $items[] = array('icon'=>'isw-left','label'=>Tk::g('Return'),'url'=>array('seas'));
